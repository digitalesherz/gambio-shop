		...
		if(gm_get_conf('GM_SHOP_OFFLINE') != 'checked' || $_SESSION['customers_status']['customers_status_id'] == 0)
		{
			...
			if($t_page == 'ProductInfo' || $t_page == 'Cart' || $t_page == 'Wishlist')
			{
				...
				if($t_no_js === false)
				{
					...
					$this->v_output_buffer['GPRINT_JAVASCRIPT_CODE'] = '<script defer type="text/javascript" src="gm_javascript.js.php?page=Section&amp;section=load_gprint&amp;globals=off&amp;current_page=' . $t_page . '&amp;mode=frontend' . $t_gm_gprint_product . $t_gm_gprint_surfaces_groups_id_parameter . $t_open_cart_dropdown . '"></script>';
				}
			}
		}
