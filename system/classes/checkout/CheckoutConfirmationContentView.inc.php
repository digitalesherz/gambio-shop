<?php
/* --------------------------------------------------------------
  CheckoutConfirmationContentView.inc.php 2016-12-20
  Gambio GmbH
  http://www.gambio.de
...
require_once(DIR_FS_INC . 'get_transfer_charge_text.inc.php');
...
class CheckoutConfirmationContentView extends ContentView
{
	...
	public function prepare_data()
	{
		...
		if(empty($t_uninitialized_array))
		{
			...
			if($coo_order->info['payment_class'] === 'cod')
			{
				$this->content_array['COD_INFO'] = get_transfer_charge_text($coo_order->info['shipping_class'], $coo_order->delivery['country']['iso_code_2'],  $coo_xtc_price->cStatus['customers_status_id'], $coo_xtc_price->actualCurr);
			}
			...
			{
				$coo_ts_excellence = MainFactory::create('TrustedShopsExcellenceContentView');
				$coo_ts_excellence->setOrder($coo_order);
