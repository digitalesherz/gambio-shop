<?php
/* --------------------------------------------------------------
  HeaderContentView.inc.php 2016-11-25
  Gambio GmbH
  http://www.gambio.de
...
class HeaderContentView extends ContentView
{
	...
	public function prepare_data()
	{
		...
		{
			$mainCssFile = file_exists(DIR_FS_CATALOG . '.dev-environment') ? 'main.css' : 'main.min.css';
			$mainCssPath = DIR_FS_CATALOG . 'templates/' . CURRENT_TEMPLATE . '/' . $mainCssFile;
			
			if(file_exists($mainCssPath))
			{
				$this->content_array['MAIN_CSS'] = $mainCssFile . '?bust=' . filemtime($mainCssPath);
			}
		}
	...
	public function get_additional_html_array()
	{
		...
		if(empty($t_uninitialized_array))
		{
			...
			if(MOBILE_ACTIVE != 'true'
			   && file_exists(DIR_FS_CATALOG . 'templates/' . CURRENT_TEMPLATE . '/javascript/general.js.php')
			)
			{
	...
	{
		$t_meta_tags_html      = '';
		$t_uninitialized_array = $this->get_uninitialized_variables(array(
			                                                            'c_path',
			                                                            'coo_product',
			                                                            'script_name'
		                                                            ));
	...
	protected function set_validation_rules()
	{
		...
		$this->validation_rules_array['style_edit_mode'] = array(
			'type'   => 'string',
			'strict' => 'true'
		);
		$this->validation_rules_array['script_name']     = array(
			'type'   => 'string',
			'strict' => 'true'
		);
		$this->validation_rules_array['c_path']          = array(
			'type'   => 'string',
			'strict' => 'true'
		);
		...
		$this->validation_rules_array['extender_html']   = array(
			'type'   => 'string',
			'strict' => 'true'
		);
		$this->validation_rules_array['coo_payment']     = array(
			'type'        => 'object',
			'object_type' => 'payment'
		);
	}
