<?php
/* --------------------------------------------------------------
  WithdrawalControl.inc.php 2017-01-19
  Gambio GmbH
  http://www.gambio.de
...
class WithdrawalControl extends DataProcessing
{
	...
	protected function send_confirmation_mail(WithdrawalModel $p_coo_withdrawal, $p_email)
	{
		...
		if(SEND_EMAILS == 'true')
		{
			...
			{
				// send mail to admin
				xtc_php_mail(EMAIL_BILLING_ADDRESS, $p_coo_withdrawal->get_customer_name(), EMAIL_BILLING_ADDRESS,
				             STORE_NAME, EMAIL_BILLING_FORWARDING_STRING, $p_email,
				             $p_coo_withdrawal->get_customer_name(), '', '', $t_subject, $t_html, $t_txt);
				
				// send mail to customer
				$t_mail_status = xtc_php_mail(EMAIL_BILLING_ADDRESS, EMAIL_BILLING_NAME, $p_email,
				                              $p_coo_withdrawal->get_customer_name(), '', EMAIL_BILLING_REPLY_ADDRESS,
				                              EMAIL_BILLING_REPLY_ADDRESS_NAME, '', '', $t_subject, $t_html, $t_txt);
			}
		}
