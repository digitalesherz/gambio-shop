<?php
/* --------------------------------------------------------------
   GxmlCustomers.inc.php 2016-09-09
   Gambio GmbH
   http://www.gambio.de
...
class GxmlCustomers extends GxmlMaster
{
	...
	protected function _createCustomerByNode(SimpleXMLElement $node)
	{
		...
		$hashedPassword = $authService->getHash(new StringType((string)$node->password));
		
		$customer->setPassword(MainFactory::create('CustomerHashedPassword', new NonEmptyStringType($hashedPassword)));
