<?php
/* --------------------------------------------------------------
   SendOrderContentView.inc.php 2016-12-16
   Gambio GmbH
   http://www.gambio.de
...
class SendOrderContentView extends ContentView
{
	...
	public function get_mail_content_array()
	{
		...
		if($this->payment_module === 'cod')
		{
			$t_cod_info = get_transfer_charge_text($this->shipping_module, $t_order->delivery['country_iso_2'], $this->order->customer['status'], $t_order->info['currency']);
			$this->set_content_data('COD_INFO', $t_cod_info);
		}
